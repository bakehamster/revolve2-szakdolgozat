...
from ..mujoco_signaler.mujoco_signaler import mujoco_signal
...
class SceneSimulator:
    ...
    _scene: Scene = None
    _headless: bool
    _record_settings: RecordSettings | None
    _start_paused: bool
    _control_step: float
    _sample_step: float | None
    _simulation_time: int | None
    ...
    _simulation_states: list[SimulationState]

    _last_control_time: float
    _last_sample_time: float
    _last_video_time: float  # time at which last video frame was saved

    def __init__(self, headless: bool,
                 start_paused: bool,
                 control_step: float,
                 sample_step: float | None,
                 cast_shadows: bool, fast_sim):
        ...
        self._last_video_time = 0.0  # time at which last video frame was saved

        # mujoco_signal.revolve_signal.connect(self._save_simulation_state)
    ...
    def simulate_scene(self,
        ...
        self._handle_scene(scene, simulation_timestep)

        if not self._headless or record_settings is not None:
            mujoco_signal.throw_signal(self._model, self._data)
    ...
    def _handle_scene(self, scene: Scene, simulation_timestep: float):
        self._model, self._mapping = scene_to_model(
            scene, simulation_timestep, cast_shadows=self._cast_shadows, fast_sim=self._fast_sim
        )
    ...
    def _save_simulation_state(self):
        ...
        if self._sample_step is not None:
            self._simulation_states.append(
                SimulationStateImpl(data=self._data, abstraction_to_mujoco_mapping=self._mapping)
            )

        control_interface = ControlInterfaceImpl(
            data=self._data, abstraction_to_mujoco_mapping=self._mapping
        )
        while (time := self._data.time) < (float("inf") if self._simulation_time is None else self._simulation_time):
            # do control if it is time
            if time >= self._last_control_time + self._control_step:
                last_control_time = math.floor(time / self._control_step) * self._control_step

                simulation_state = SimulationStateImpl(
                    data=self._data, abstraction_to_mujoco_mapping=self._mapping
                )
                self._scene.handler.handle(simulation_state, control_interface, self._control_step)

            # sample state if it is time
            if self._sample_step is not None:
                if time >= self._last_sample_time + self._sample_step:
                    last_sample_time = int(time / self._sample_step) * self._sample_step
                    self._simulation_states.append(
                        SimulationStateImpl(
                            data=self._data, abstraction_to_mujoco_mapping=self._mapping
                        )
