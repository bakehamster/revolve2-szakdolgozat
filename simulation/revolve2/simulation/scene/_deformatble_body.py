...
from .geometry import Geometry, GeometryBox, GeometryCylinder
...
class DeformableBody:
    ...
    def inertia_tensor(self) -> Matrix33:
        ...
        for geometry in self.geometries:
            ...
            elif isinstance(geometry, GeometryCylinder):
                # Handle Cylinder geometry
                # For simplicity, assuming the cylinder is aligned with the local axes
                radius = geometry.size.x / 2
                height = geometry.size.y
                mass = geometry.mass

                # Moment of inertia for a cylinder aligned with the local axes
                ixx = (1 / 12) * mass * (3 * radius ** 2 + height ** 2)
                iyy = (1 / 12) * mass * (3 * radius ** 2 + height ** 2)
                izz = (1 / 2) * mass * radius ** 2

                local_inertia = Matrix33([[ixx, 0, 0], [0, iyy, 0], [0, 0, izz]])
